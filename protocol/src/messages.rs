...
/* TODO: annoying that this isn't generated by prost-build. */
#[tonic::async_trait]
pub trait OperationService {
  /* TODO: The IDE's VFS is an OperationService!!! */
  async fn process_op(&self, request: Operation) -> Result<OperationResult, ProtocolError>;
}
...
pub struct IDEServiceClient {
  op_dispatcher: OperationServiceClient,
}
...
impl IDEService for IDEServiceClient {
  async fn process_ide_msg(&self, request: IDEMessage) -> Result<ClientMessage, ProtocolError> {
    ...
    let client_msg = match request {
      IDEMessage::op(op) => {
        let result = self.op_dispatcher.process_op(op).await?;
