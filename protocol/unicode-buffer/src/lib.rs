/*
 * Description: A mutable string supporting editing operations over unicode code points.
 *
...
//! A mutable string supporting editing operations over unicode code points.
...
impl Default for InternedTexts {
...
impl InternedTexts {
  ...
  pub fn decrement(&mut self, checksum: TextChecksum) -> Result<(), InternError> {
    let text_section = self
      .interned_text_sections
      .get_mut(&checksum)
      .ok_or(InternError::ChecksumDidNotExist(checksum))?;
    text_section.num_occurrences -= 1;
...
///
///```
/// use unicode_buffer::{Tokenizer, NewlineTokenizer};
///
...
impl Buffer {
  ...
  ///
  ///```
  /// # fn main() -> Result<(), unicode_buffer::BufferError> {
  /// use unicode_buffer::*;
  ///
  ...
  /// let index = ab_c.locate_code_point(CodePointInsertionIndex(1))?;
  /// assert_eq!(index, InsertionIndex(1));
  /// let index = ab_c.locate_code_point(CodePointInsertionIndex(2))?;
  /// assert_eq!(index, InsertionIndex(2));
  ///
  /// let ae_o = Buffer::tokenize("a̐é\nö̲g")?;
  /// let index = ae_o.locate_code_point(CodePointInsertionIndex(2))?;
  /// assert_eq!(index, InsertionIndex(3));
  /// let index = ae_o.locate_code_point(CodePointInsertionIndex(4))?;
  /// assert_eq!(index, InsertionIndex(6));
  /// # Ok(())
  /// # }
  ///```
  pub fn locate_code_point(
    &self,
    at: CodePointInsertionIndex,
  ) -> Result<InsertionIndex, BufferError> {
